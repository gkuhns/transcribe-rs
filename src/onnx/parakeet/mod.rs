...
impl ParakeetModel {
    pub fn load(model_dir: &Path, quantization: &Quantization) -> Result<Self, TranscribeError> {
        ...
        let decoder_joint = session::create_session_decoder(&decoder_path)?;
        let preprocessor = session::create_session_encoder(&preprocessor_path)?;
        ...
        let text = match &*DECODE_SPACE_RE {
            Ok(regex) => regex
                .replace_all(&tokens.join(""), |caps: &regex::Captures| {
                    if caps.get(1).is_some() { " ".to_string() } else { "".to_string() }
                })
                .to_string(),
            Err(_) => tokens.join(""),
        };
...
impl SpeechModel for ParakeetModel {
    fn capabilities(&self) -> ModelCapabilities { CAPABILITIES }
    fn default_leading_silence_ms(&self) -> u32 { Self::DEFAULT_LEADING_SILENCE_MS }
    fn transcribe_raw(
        &mut self,
        samples: &[f32],
...
fn convert_timestamps(
    ...
    match granularity {
        ...
        TimestampGranularity::Segment => convert_to_hierarchical_segment_segments(timestamped_result),
    }
}

fn convert_to_raw_token_segments(timestamped_result: &TimestampedResult) -> Vec<TranscriptionSegment> {
    let mut segments = Vec::new();
    for (i, (token, &timestamp)) in timestamped_result.tokens.iter().zip(timestamped_result.timestamps.iter()).enumerate() {
        let end_timestamp = timestamped_result.timestamps.get(i + 1).copied().unwrap_or(timestamp + 0.05);
        segments.push(TranscriptionSegment { start: timestamp, end: end_timestamp, text: token.clone() });
    }
    segments
}

fn convert_to_hierarchical_word_segments(timestamped_result: &TimestampedResult) -> Vec<TranscriptionSegment> {
    if timestamped_result.tokens.is_empty() || timestamped_result.timestamps.is_empty() { return Vec::new(); }
    let tokens = create_tokens_from_timestamped_result(timestamped_result);
    let words = group_tokens_into_words(&tokens);
    words.iter().filter(|w| !w.text.trim().is_empty()).map(|w| TranscriptionSegment { start: w.t_start, end: w.t_end, text: w.text.clone() }).collect()
}

fn convert_to_hierarchical_segment_segments(timestamped_result: &TimestampedResult) -> Vec<TranscriptionSegment> {
    if timestamped_result.tokens.is_empty() || timestamped_result.timestamps.is_empty() { return Vec::new(); }
    let tokens = create_tokens_from_timestamped_result(timestamped_result);
    let words = group_tokens_into_words(&tokens);
    let segments = group_words_into_segments(&words);
    segments.iter().filter(|s| !s.text.trim().is_empty()).map(|s| TranscriptionSegment { start: s.t_start, end: s.t_end, text: s.text.clone() }).collect()
}

fn create_tokens_from_timestamped_result(timestamped_result: &TimestampedResult) -> Vec<Token> {
    timestamped_result.tokens.iter().zip(timestamped_result.timestamps.iter()).enumerate().map(|(i, (token_text, &timestamp))| {
        let t_end = timestamped_result.timestamps.get(i + 1).copied().unwrap_or(timestamp + 0.05);
        Token { text: token_text.clone(), t_start: timestamp, t_end, is_blank: token_text.trim().is_empty() }
    }).collect()
}
...
fn group_tokens_into_words(tokens: &[Token]) -> Vec<Word> {
    ...
    for token in tokens {
        ...
        let starts_new_word = token.text.starts_with(' ') || token.text.starts_with('▁') || (current_word_tokens.is_empty() && !token.text.trim().is_empty());
        if starts_new_word && !current_word_tokens.is_empty() {
            let word = create_word_from_tokens(&current_word_tokens);
            if !word.text.is_empty() { words.push(word); }
...
fn create_word_from_tokens(tokens: &[Token]) -> Word {
    if tokens.is_empty() {
        return Word { text: String::new(), t_start: 0.0, t_end: 0.0, tokens: Vec::new() };
    }
    let t_start = tokens.first().unwrap().t_start;
    let t_end = tokens.last().unwrap().t_end;
    let text = tokens.iter().map(|t| {
        if t.text.starts_with('▁') { t.text.strip_prefix('▁').unwrap_or(&t.text) }
        else if t.text.starts_with(' ') { t.text.strip_prefix(' ').unwrap_or(&t.text) }
        else { &t.text }
    }).collect::<String>().trim().to_string();
    Word { text, t_start, t_end, tokens: tokens.to_vec() }
}
...
fn group_words_into_segments(words: &[Word]) -> Vec<Segment> {
    ...
    for (i, word) in words.iter().enumerate() {
        ...
        let ends_segment = word.text.chars().any(|c| segment_separators.contains(&c)) || i == words.len() - 1;
        if ends_segment && !current_words.is_empty() {
            let text = current_words.iter().map(|w| w.text.as_str()).filter(|s| !s.is_empty()).collect::<Vec<_>>().join(" ");
            if !text.is_empty() {
                segments.push(Segment { text, t_start: current_words.first().unwrap().t_start, t_end: current_words.last().unwrap().t_end });
            }
            current_words.clear();
        }
    }
    if segments.is_empty() && !words.is_empty() {
        let text = words.iter().map(|w| w.text.as_str()).filter(|s| !s.is_empty()).collect::<Vec<_>>().join(" ");
        if !text.is_empty() {
            segments.push(Segment { text, t_start: words.first().unwrap().t_start, t_end: words.last().unwrap().t_end });
        }
    }
    segments
}
